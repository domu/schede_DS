...
<style>
    ...
    /* Singole voci all'interno del menu a tendina scuro */
    ...
    /* Hover e selezione sulle voci dei dropdown */
    ...
    /* Pulsanti principali personalizzati */
    div.stButton > button {
...
def estrai_asin(url):
    """Estrae l'ASIN univoco di 10 caratteri dall'URL fornito."""
...
def genera_fallback_generico(url, asin):
    """Genera una scheda vuota per consentire la compilazione manuale dei dati."""
    return {
        "asin": asin,
...
with st.expander("📂 Importazione Massiva Link (Carica file Excel, CSV o File di Testo)"):
    st.write("Carica un file esportato con i tuoi link. L'applicazione rileverà automaticamente tutti i collegamenti Amazon validi.")
    ...
    if file_caricato is not None:
        ...
        try:
            ...
            amazon_urls = list(set(amazon_urls)) # rimuove i duplicati
            
            if amazon_urls:
                elaborati = 0
                with st.spinner(f"Elaborazione di {len(amazon_urls)} link..."):
                ...
                st.error("Nessun link di Amazon.it rilevato all'interno del file.")
...
def genera_pdf_esportabile(lista_prodotti):
    ...
    # Forza la conversione in bytes nativi per evitare crash nel modulo download_button di Streamlit
...
with col_download_pdf:
    ...
    if pdf_finito is not None:
        ...
        st.warning("Aggiungi 'fpdf2' al tuo file requirements.txt per abilitare il download PDF")
